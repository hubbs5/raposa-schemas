...
signal_count = ['First', 'Second', 'Third', 'Fourth',
...
every_indicator = {'Stop Profit': 'STOP_PRICE',  # uses same schema class, but input numbers are constrained to +
...
indicator_comps = {'Price': 'PRICE',
                   'SMA': 'SMA',
                   'EMA': 'EMA',
                   'MACD': 'MACD'}

indicator_comps_price = {'SMA': 'SMA',
                         'EMA': 'EMA',
                         'MACD': 'MACD'}

sell_indicators = {'Stop Profit': 'STOP_PRICE',  # uses same schema class, but input numbers are constrained to +
                   # uses same schema class, but input numbers are constrained to -
                   'Stop Loss': 'STOP_PRICE',
                   'ATR Stop Profit': 'ATR_STOP_PRICE', 
                   'ATR Stop Loss': 'ATR_STOP_PRICE', 
...
indicators_with_time_params = {'SMA': ['period'], #Indicators that have to look back in time and the param(s)
...
class MACD(BaseModel):
        ...
        if len(key_standard) != len(value):
            ...
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    ...
                    raise TypeError(
                        'MACD periods must be numbers')
                elif not value[key] > 0 :
                    raise TypeError(
                        'MACD inputs must be greater than zero')
        return value
    ...
    def fast_slow_comparison(cls, value, values):
        if value['slowEMA_period'] <= value['fastEMA_period']:
            raise ValueError(
                'MACD fast EMA period must be less than slow EMA period')
        return value
        ...
        if len(key_standard) != len(value):
            ...
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    ...
                    raise TypeError(
                        'MACD Signal periods must be numbers')
                elif not value[key] > 0 :
                    raise TypeError(
                        'MACD Signal inputs must be greater than zero')
        return value
        ...
        if len(key_standard) != len(value):
            ...
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    ...
                    raise TypeError(
                        'RSI period must be a number')
                elif not value[key] > 0 :
                    raise TypeError(
                        'RSI inputs must be greater than zero')
        return value

class STOP_PRICE(BaseModel):
    name: str = 'STOP_PRICE'
    params: dict = {"percent_change": 10.1} # will be positive for stop profit and neg for stop loss
    ...
    def check_valid_percent(cls, value, values, **kwargs):
        if value == 'Stop Profit' and values['params']['percent_change'] < 0:
            raise ValueError(
                'Stop Profit signal must have positive percentage')
        elif value == 'Stop Loss' and values['params']['percent_change'] > 0:
            raise ValueError(
                'Stop Loss signal must have negative percentage')
        return value
    ...
    def param_key_check(cls, value):
        ...
        if len(key_standard) != len(value):
            raise ValueError('wrong number of parameters used to build stop loss or profit signal')
        else:
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    raise ValueError(
                        'Wrong parameters fed to stop signal builder - please contact us about this bug')
                elif not isinstance(value[key], float) and not isinstance(value[key], int):
                    raise TypeError(
                        'stop loss or stop profit % must be a number')
        return value

class ATR_STOP_PRICE(BaseModel): 
    name: str = 'ATR_STOP_PRICE'
    params: dict = dict(period=20, 
                entry_price = 10, 
...
class PRICE(BaseModel):
    name: str = 'PRICE'
    params: dict = {"price_type":"Close"} #"High", "Low", or "Close" (always use Adj Close)
    needs_comp: bool = True
        ...
        if len(key_standard) != len(value):
            ...
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    ...
                    raise TypeError(
                        'ATR period and multiple must be a number')
                elif not value[key] > 0 :
                    raise TypeError(
        ...
        if len(key_standard) != len(value):
            ...
            for n, key in enumerate(value.keys()):
                if key != key_standard[n]:
                    ...
                    raise TypeError(
                        'RSI level must be a number')
                elif not value[key] > 0 :
                    raise TypeError(
                        'RSI level input must be greater than zero')
        return value
...
class Signal(BaseModel):
    ...
    def comp_indicator_check(cls, value, values, **kwargs):
        ...
        if value is None and values['indicator']['needs_comp']:
            raise ValueError(
                'Indicator chosen needs comparison operator')
        ...
        # We also make sure that the provided comp indicator is valid to compare with the first indicator
            ...
            if value['name'] not in values['indicator']['valid_comps']:
                raise ValueError(
                    'invalid comparison indicator chosen for indicator')
        return value
